...
מטרת המאמר אינה להסביר כיצד לפתוח קבצי Apk, וכדי לא להאריך את המאמר יתר על המידה, ברשותכם, אפנה אתכם לפוסט שכתבתי המסביר בפירוט כיצד לבצע את זה, בעזרת התוכנה Apktool. [Analyze an Apk file]({% post_url 2012-11-18-analyze-an-apk-file %}).
...
כמו שמצויין בהקדמה, מטרת המאמר אינה להסביר כיצד לפתוח קבצי Apk, ולכן, ברשותכם, אפנה אתכם לפוסט שכתבתי המסביר בפירוט כיצד לבצע את זה, בעזרת התוכנה Apktool. [Analyze an Apk file]({% post_url 2012-11-18-analyze-an-apk-file %}).
...
בנוסף, יש לציין כי במערכת עצמה ניתן לשנות מגוון נוסף של דברים:
  ...
  * אנימציית ההפעלה של המערכת, נמצאת בנתיב: `system/media/` והעריכה שלה פשוטה ביותר.
  ...
  * אם רוצים לשנות את כפתורי המגע (במכשירי הנקסוס), אלה הן תמונות הנמצאות ב SystemUI.apk.
...
מכיוון שאפליקציית החייגן גדולה ומורכבת, בואו נפריד את הקובץ DW.apk שבניתי בשביל ההדגמה מהסעיף הקודם ("מנקודות המבט של המפתח"), זהו קובץ פשוט שלא עושה כלום חוץ מלהציג Activity אחד ובו רשום `!Hello world`, את הקובץ ניתן להוריד [מכאן](http://www.lifelongstudent.io/files/DW.apk).
