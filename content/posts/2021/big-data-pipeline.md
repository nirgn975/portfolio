...
Now let's ask some graph questions, like

&nbsp;

## 3. Move on the cloud

After we feel a bit more comfortable with ArangoDB on our local machine, it's time to create a ArangoDB Cluster on their cloud platform.

&nbsp;

## 4. Pipeline Research

Until now we only talk about the database, which is important don't get me wrong, but it's not the main reason we're here. The title of the post is is _"Big Data **Pipeline**"_, so we need to talk about that pipeline.
